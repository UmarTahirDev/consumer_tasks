...
app.post('/api/consumers', async (req, res) => {
  const { username, name, relationship, role, ageGroup, emergency_contact, password, preferenceForms, admin_id } = req.body;
  
  try {
    console.log(role);
    
    // Check if username already exists
    const userCheck = await pool.query('SELECT * FROM users WHERE user_name = $1', [username]);
